...
def update_lesson(
    ...
    course = (
        db.query(models.Course)
        .filter(models.Course.id == db_lesson.course_id)
        .first()
    )
    if course.instructor_id != current_user.id and current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Недостаточно прав")
...
def enroll_course(
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="Вы уже записаны на этот курс"
        )
...
def clear_chat_history(
    ...
    """
    Удаляет историю чата текущего пользователя (опционально — по конкретному курсу).
    """
    query = db.query(models.ChatMessage).filter(
        models.ChatMessage.user_id == current_user.id
    )
...
@app.get("/api/my-materials", response_model=List[schemas.Material])
def get_my_materials(
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """
    Простая версия: возвращаем ВСЕ материалы в системе,
    отсортированные по дате (у тебя сейчас один пользователь — этого достаточно).
    Если потом захочешь – можно будет фильтровать по пользователю/курсам.
    """
    materials = db.query(models.CourseMaterial).order_by(
        models.CourseMaterial.created_at.desc()
    ).all()
    return materials
...
def create_assignment(
    ...
    db_assignment = models.Assignment(
        **assignment.dict(), student_id=current_user.id
    )
...
async def submit_assignment(
    ...
    check_result = await AIService.check_assignment(
        assignment.description, submission.submission
    )
...
def get_my_assignments(
    ...
    assignments = (
        db.query(models.Assignment)
        .filter(models.Assignment.student_id == current_user.id)
...
def create_schedule(
    ...
    db_schedule = models.Schedule(**schedule.dict(), user_id=current_user.id)
    db.add(db_schedule)
    db.commit()
...
async def generate_schedule(
    ...
    lessons = (
        db.query(models.Lesson)
        .filter(models.Lesson.course_id == request.course_id)
        .all()
    )
    ...
    course_info = {
        ...
        "total_duration": sum(
            [l.duration_minutes or 60 for l in lessons]
        ),
    }

    schedule_data = await AIService.generate_learning_schedule(
        course_info, request.preferences
    )

    schedules: list[models.Schedule] = []
    start_date = datetime.fromisoformat(
        request.preferences.get("start_date", datetime.now().isoformat())
    )
